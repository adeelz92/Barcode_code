...
def crop(rect, im_erosion):
    ...
    if height < width / 2 and width / height < 4:
        ...
        if y < 250:
            ...
            digits = detect(x_parts, y_parts, im_crop)
        else:
...
def detect(x_parts, y_parts, im_crop):
    ...
    for i in range(len(y_parts) - 1):
        for j in range(len(x_parts) - 1):
            ...
            if (digit.isdigit()):
                digits.append(digit)
                cv2.imwrite("./Crops/"+ IMAGE_NAME + "." + str(time.time()) + ".jpg", color_patch)
...
if __name__ == '__main__':
    ...
    images_list = glob.glob("D:\\Adeel\\barcode_ocr\\DataGather\\20181019sample\\my/*")
    for image in images_list:
        ...
        for angle in range(0, 24, 3):
            digits_from_1 = detect_digits(im, angle)
            if len(digits_from_1) is 16:
                break

            digits_from_2 = detect_digits(im, 360-angle)
